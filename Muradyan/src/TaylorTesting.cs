...
namespace TestingSystem
{
    ...
    public class MethodForTestingTaylor : MethodForTesting
    {
        ...
        public static MethodDeclarationSyntax getChangedNode(MethodDeclarationSyntax method)
        {
            ...
            method = method.ReplaceNode(parlist, newparlist);

            /* Adding __tst.AddElement(i); */
            while (true)
            {
                var desc = method.Body.DescendantTrivia().Where((t) => t.IsKind(SyntaxKind.SingleLineDocumentationCommentTrivia));
                ...
                foreach (var s in desc)
                {
                    ...
                    var lineToAdd = "\r\n"+TestEntryArgName + ".AddElement(" + ar[1] + ");\r\n";
                    method = CSharpSyntaxTree.ParseText(method.GetText().Replace(s.FullSpan, lineToAdd))
