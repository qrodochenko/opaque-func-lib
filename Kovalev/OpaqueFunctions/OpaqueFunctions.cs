...
namespace OpaqueFunctions
{
    ...
    public static class CMathApprox_Ln
    {
        ...
        public static double MathApprox_ln_4(double arg, double error)
        {
            int L = 0;
            int M = 1;
            ...
            CFind_Pade.Find_Denominator(out Ln_Denom, C, L, M);
            CFind_Pade.Find_Numerator(out Ln_Numer, Ln_Denom, C, L, M);
            return Ln_Denom[L];
    ...
    {
        public static void Find_Denominator(out double[] Find, double[] Taylor, int L, int M)
        {
            Find = new double[M + 1];
        ...
        public static void Find_Numerator(out double[] Find, double[] Denom, double[] Taylor, int L, int M)
        {
            Find = new double[L + 1];
            int N = Math.Min(L, M);
            for (int i = 0; i <= L; i++)
